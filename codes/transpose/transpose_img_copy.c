...
int main(int argc, char *argv[])
{
	...
	img_desc.image_width = ncols_orig;
	img_desc.image_height = nrows_orig;
	cl_mem d_base = clCreateImage(ctx, CL_MEM_READ_WRITE, &img_fmt, &img_desc, NULL, &err);
	ocl_check(err, "clCreateImage fallito (base)");
	...
	// Copia da buffer a immagine
	const size_t dst_origin[] = {0, 0, 0};
	const size_t region[] = {ncols_orig, nrows_orig, 1};
	cl_event copy_evt;
	err = clEnqueueCopyBufferToImage(que, d_orig, d_base, 0, dst_origin, region,
		1, &init_evt, &copy_evt);
	ocl_check(err, "enqueue copy buffer to image");

    cl_int nrows_tras = ncols_orig;
    cl_int ncols_tras = nrows_orig;
    cl_event transpose_evt = transpose(que, transpose_k, d_tras, d_base, nrows_tras, ncols_tras, lws, copy_evt);

	cl_event wait_list[] = { init_evt, copy_evt, transpose_evt };
	cl_event read_evt;

	cl_int * h_tras = clEnqueueMapBuffer(que, d_tras, CL_TRUE, CL_MAP_READ, 0, memsize, 3, wait_list, &read_evt, &err);
	...
	cl_ulong copy_ns = runtime_ns(copy_evt);
	...
	printf("copy: %.4gms %.4gGB/s\n", copy_ns*1.0e-6, 2*memsize/(double)copy_ns);
