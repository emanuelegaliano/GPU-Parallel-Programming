// Implementation of vecsmooth with vectorization and local memory usage
	...
	size_t lws_cli, cl_event init_evt)
{
	...
	ocl_check(err, "smooth_k set kernel arg %u", arg_index);
    ++arg_index;
	err = clSetKernelArg(smooth_k, arg_index, lws_cli*sizeof(cl_int), NULL);
	ocl_check(err, "smooth_k set kernel arg %u", arg_index);
    ++arg_index;
	err = clSetKernelArg(smooth_k, arg_index, lws_cli*sizeof(cl_int), NULL);
	ocl_check(err, "smooth_k set kernel arg %u", arg_index);
...
int main(int argc, char *argv[])
{
	...
	cl_kernel smooth_k = clCreateKernel(prog, "smooth_v4_lmem_k", &err);
	ocl_check(err, "clCreateKernel smooth_v4_lmem_k fallito");
