// Implementation of vecsmooth with local memory usage
	...
	size_t lws_cli, cl_event init_evt)
{
	...
	ocl_check(err, "smooth_k set kernel arg %u", arg_index);
    ++arg_index;
	// NULL -> indica solo di allocare (lws_cli+2)*sizeof(cl_int) byte di spazio nella local memory
	err = clSetKernelArg(smooth_k, arg_index, (lws_cli+2)*sizeof(cl_int), NULL);
	ocl_check(err, "smooth_k set kernel arg %u", arg_index);
...
int main(int argc, char *argv[])
{
	...
	cl_kernel smooth_k = clCreateKernel(prog, "smooth_lmem_k", &err);
	ocl_check(err, "clCreateKernel smooth_lmem_k fallito");

	cl_event init_evt = init(que, init_k, d_in, nels, lws);

	cl_event smooth_evt = smooth(que, smooth_k, d_out, d_in, nels, lws, init_evt);
	...
	printf("smooth: %.4gms %.4gGB/s, %.4gGE/s\n", smooth_ns*1.0e-6, 4*memsize/(double)smooth_ns, nels/(double)smooth_ns);
