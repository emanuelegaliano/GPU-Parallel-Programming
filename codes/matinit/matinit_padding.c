/* Host matinit code with padding implementation */

#include <stdio.h>
#include <stdlib.h>
#include "../ocl_boiler.h"

cl_event init(cl_command_queue que, cl_kernel init_k, cl_mem mat, cl_int nrows, cl_int ncols, cl_int pitch_el, cl_int lws_cli)
{
	cl_int err;
	cl_event init_evt;
	...
	ocl_check(err, "init_k set kernel arg %u", arg_index);
	++arg_index;
	err = clSetKernelArg(init_k, arg_index, sizeof(pitch_el), &pitch_el);
	ocl_check(err, "init_k set kernel arg %u", arg_index);
...
int main(int argc, char *argv[])
{
	...
	// Calcolo del base alignment
	cl_uint base_alignment_bit;
	ocl_check(clGetDeviceInfo(d, CL_DEVICE_MEM_BASE_ADDR_ALIGN, sizeof(base_alignment_bit), &base_alignment_bit, NULL), "get base address alignment");
	const size_t base_alignment_byte = base_alignment_bit/8;
	const size_t base_alignment_el = base_alignment_byte/sizeof(cl_int);
	// Verifica che l'allineamento sia un multiplo della dimensione di un cl_int
	if (base_alignment_el*sizeof(cl_int) != base_alignment_byte) {
		printf("Per favore usa una architettura decente\n");
	}
	// Calcolo pitch della matrice in elementi
	size_t padded_pitch_el = round_mul_up(ncols, base_alignment_el);

	const size_t memsize = padded_pitch_el*nrows*sizeof(cl_int); // dimensione della matrice con padding
	const size_t natsize = ncols*nrows*sizeof(cl_int); // dimensione "naturale" della matrice
	...
	const char *kname = "init_padding_k";
	cl_kernel init_k = clCreateKernel(prog, kname, &err);
	ocl_check(err, "clCreateKernel %s fallito", kname);

	cl_event init_evt = init(que, init_k, d_vec, nrows, ncols, padded_pitch_el, lws);
	...
	verify(h_vec, nrows, ncols, padded_pitch_el);
	...
	printf("init: %.4gms %.4gGB/s\n", init_ns*1.0e-6, natsize/(double)init_ns);
