\chapter{Basi di OpenCL}\label{chap:opencl_basi}
...
Il buffer è una regione di memoria allocata sul dispositivo, che può essere utilizzata per scambiare dati tra il host e il device. Per creare un buffer, è necessario specificare la dimensione del buffer, le opzioni di accesso, il puntatore ai dati (che generalmente è \texttt{NULL} se non si vuole usare un puntatore host) e un puntatore per eventuali errori. Il codice seguente mostra come creare un buffer:
...
\begin{itemize}
    ...
    \item \texttt{ocl\_check(err, "clCreateBuffer fallito")} è una funzione di utilità che verifica il valore di \texttt{err}. Se il codice di errore è diverso da \texttt{CL\_SUCCESS}, viene segnalato l’errore e l’esecuzione del programma viene interrotta.
\end{itemize}

\section{Creazione del kernel}\label{sec:opencl_basi_kernel}
Il kernel è quella funzione che viene eseguita sul dispositivo. Per creare un kernel, è necessario specificare il nome del kernel e il programma da cui è stato creato. Il codice seguente mostra come creare un kernel:
...
\begin{code}[caption={Esecuzione di un kernel OpenCL}, label={code:opencl_basi_execution}]
  ...
  err = clEnqueueNDRangeKernel(que, init_k,
    1,    /* dimensionalit\`a griglia: nel nostro caso 1D */
...
Dopo l'esecuzione del kernel, è necessario recuperare i dati dal dispositivo al host. Per fare questo, si utilizza la funzione \texttt{clEnqueueReadBuffer}, che legge i dati da un buffer OpenCL e li copia in una regione di memoria host. Il codice seguente mostra come fare questo:
...
\begin{itemize}
    ...
    \item Il controllo:
    \begin{itemize}
        \item \texttt{if (!h\_vec)} verifica che l’allocazione sia andata a buon fine.
        \item In caso contrario, viene stampato un messaggio di errore e l’esecuzione del programma viene terminata.
    \end{itemize}
    
    \item \texttt{cl\_event wait\_list[1] = \{ init\_evt \}} è un array di eventi che rappresenta la lista di comandi che devono essere completati prima di eseguire la lettura. In questo caso, si aspetta il completamento del kernel associato all’evento \texttt{init\_evt}.
    ...
    \item \texttt{ocl\_check(err, "read buffer d\_vec")} verifica che l’operazione sia andata a buon fine. In caso di errore, viene segnalato e l’esecuzione viene interrotta.
\end{itemize}

\section{Ottimizzazione con il mapping}\label{sec:opencl_basi_mapping}

Nella versione precedente, dopo l’esecuzione del kernel, i dati prodotti sul device venivano trasferiti all’host tramite una copia esplicita. Questo richiedeva due operazioni:
\begin{itemize}
    \item allocare un vettore lato host con \texttt{malloc};
    \item copiare i dati con \texttt{clEnqueueReadBuffer}.
\end{itemize}
\noindent
Questo approccio è semplice ma introduce un overhead dovuto sia alla gestione separata della memoria sia alla copia esplicita dei dati.

L’idea del mapping è quella di evitare la copia esplicita dei dati e permettere all’host di accedere direttamente al buffer OpenCL. In pratica:
\begin{itemize}
    ...
    \item al termine dell’uso, il buffer viene rilasciato con \texttt{clEnqueueUnmapMemObject}.
\end{itemize}

\subsection{Modifica nella creazione del buffer}
...
Il flag \texttt{CL\_MEM\_ALLOC\_HOST\_PTR} indica al runtime di allocare il buffer in modo tale da poter essere mappato dall’host.

\subsection{Recupero dei dati con il mapping}
La seconda modifica riguarda il recupero dei dati. Nella versione precedente si utilizzava \texttt{clEnqueueReadBuffer}, mentre nella nuova versione si utilizza:
...
Questa funzione restituisce direttamente un puntatore \texttt{h\_vec} utilizzabile dal codice host.

\subsection{Accesso ai dati}
Una volta ottenuto il puntatore, i dati possono essere utilizzati direttamente:
...
\noindent
Non è più necessario effettuare una copia esplicita dei dati.

\paragraph{Problemi di swap e pinning.}
Generalmente, per ottenere trasferimenti efficienti tra host e device, il runtime OpenCL utilizza operazioni di DMA (Direct Memory Access), che permettono di copiare i dati senza coinvolgere direttamente la CPU. Questo tipo di trasferimento è asincrono rispetto all’host.

Il problema nasce dal fatto che la memoria dell’host è normalmente gestita dal sistema operativo e può essere soggetta a operazioni come lo swap o la rilocazione delle pagine. Se durante un trasferimento DMA il sistema operativo sposta o modifica quella regione di memoria, si possono verificare problemi di coerenza dei dati o addirittura errori.

Per questo motivo è necessario garantire che la memoria coinvolta nel trasferimento rimanga stabile per tutta la durata dell’operazione. Questo si ottiene tramite il \emph{pinning}, ovvero bloccando (page-locking) le pagine di memoria dell’host in modo che non possano essere spostate dal sistema operativo. Questo viene fatto grazie alla flag \texttt{CL\_MEM\_ALLOC\_HOST\_PTR} durante la creazione del buffer, che indica al runtime di allocare la memoria in modo compatibile con il pinning.

Inoltre, è importante che l’host non acceda alla memoria mappata finché l’operazione asincrona non è completata. Questo può essere garantito utilizzando eventi OpenCL oppure operazioni bloccanti, evitando così condizioni di race e inconsistenze nei dati.

\subsection{Fase di unmapping}
...
Questa operazione segnala al runtime che l’host ha terminato di accedere al buffer.