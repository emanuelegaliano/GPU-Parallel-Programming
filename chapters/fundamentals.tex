...
Nel caso di CUDA, il linguaggio è fortemente integrato con il modello di esecuzione, ma presenta alcune limitazioni: ad esempio, non esiste una vera libreria standard completa e non sono supportate alcune operazioni tipiche dei linguaggi vettoriali, come lo \emph{swizzling}. Inoltre, l’indice globale di un thread deve essere calcolato manualmente combinando gli indici di blocco e thread.

OpenCL, invece, fornisce un linguaggio più ricco dal punto di vista delle operazioni vettoriali. Sono disponibili costrutti come lo swizzling (ad esempio \texttt{f.xy}, \texttt{f.wwxx}) e funzioni built-in per ottenere direttamente gli indici dei work-item e dei work-group, semplificando la scrittura del codice parallelo.