...
\node[above, font=\small\bfseries] at (15,0.5) {Trasposta};

%% ---- Elemento s: (r=0, c=3) nell'originale → (r=3, c=0) nella trasposta ----
\fill[black!85] (3.5,-0.5) circle (0.40);
\node[font=\small\bfseries, white] at (3.5,-0.5) {$s$};
\node[above, font=\small, fill=white, fill opacity=0.9, inner sep=2pt]
    at (3.5, 0.1) {$(0,\,3)$};

%% Elemento x: (r=1, c=5) → (r=5, c=1) [li_r=1, li_c=2 → tile[1*lws+2]]
\fill[black!85] (5.5,-1.5) circle (0.40);
\node[font=\small\bfseries, white] at (5.5,-1.5) {$x$};
\node[above, font=\small, fill=white, fill opacity=0.9, inner sep=2pt]
    at (5.5,-0.9) {$(1,\,5)$};

%% ---- Fase 1: frecce load (originale → bordo sinistro del tile) ----
\draw[->, thick, black!75] (3.5,-0.5) to[out=0, in=155] (7.5,-2);
\draw[->, thick, black!75] (5.5,-1.5) to[out=0, in=155] (7.5,-3);
\node[font=\small\bfseries, black!80, align=center,
      draw=gray!50, fill=white, inner sep=3pt, rounded corners=2pt]
    at (5.8, 1.5) {Fase~1\\ letture coal.};

%% ---- Percorso nella tile (mostra la trasposizione degli indici) ----
\draw[gray!55, thin, dashed] (7.5,-2) -- (10.5,-2);
\draw[gray!55, thin, dashed] (7.5,-3) -- (10.5,-4);

%% ---- Fase 2: frecce store (bordo destro del tile → trasposta) ----
\draw[->, thick, olive!80!black] (10.5,-2) to[out=25, in=150] (12.5,-3.5);
\draw[->, thick, olive!80!black] (10.5,-4) to[out=25, in=150] (13.5,-5.5);
\node[font=\small\bfseries, olive!80!black, align=center,
      draw=gray!50, fill=white, inner sep=3pt, rounded corners=2pt]
    at (11.5,-6.8) {Fase~2\\ scritture coal.};

%% ---- Elementi nella trasposta ----
\fill[black!85] (12.5,-3.5) circle (0.40);
\node[font=\small\bfseries, white] at (12.5,-3.5) {$s$};
\node[below, font=\small, fill=white, fill opacity=0.9, inner sep=2pt]
    at (12.5,-4.1) {$(3,\,0)$};

\fill[black!85] (13.5,-5.5) circle (0.40);
