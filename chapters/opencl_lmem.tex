...
In questa versione ogni work-item carica il proprio elemento in una cache condivisa di \texttt{lws+2} elementi. Il first work-item del work-group carica anche il predecessore del work-group, l'ultimo carica anche il successore. Una barriera garantisce che la cache sia completamente inizializzata prima che qualsiasi work-item la legga.
...
L'ultima versione combina vettorizzazione e local memory. Poiché ogni work-item opera su una quartina, gli unici elementi che possono essere utili ai work-item vicini sono il primo (\texttt{.x}) e l'ultimo (\texttt{.w}) componente di ciascuna quartina. Si usano due cache separate di \texttt{lws} elementi ciascuna:
...
Lato host si allocano due cache di \texttt{lws\_cli} elementi ciascuna — senza il \texttt{+2} della versione scalare, poiché le cache contengono già esattamente un elemento per work-item. Come nella versione vettoriale base, si passa \texttt{nels/4} al kernel:
